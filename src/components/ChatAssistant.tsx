  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-20 right-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-brand-600 text-white shadow-lg transition hover:bg-brand-700 lg:bottom-6 ${
          open ? "hidden" : "flex"
        }`}
      ...
      {/* Chat panel */}
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 lg:items-center">
          <div className="flex h-[70vh] w-full max-w-md flex-col rounded-t-2xl bg-white shadow-xl lg:h-[600px] lg:rounded-2xl">
            {/* Header */}
            ...
            <form
              ...
              className="flex items-center gap-2 border-t border-slate-100 px-4 py-3 safe-bottom"
            >
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask about today..."
                className="flex-1 rounded-full border border-slate-200 px-4 py-2 text-sm focus:border-brand-400 focus:outline-none"
              />
              <button
                type="submit"
